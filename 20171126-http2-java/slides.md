...
<img src="./naver-main.png" style="max-height:400px" />

---

<img src="./naver-cafe.png" style="max-height:460px" />

---

<img src="./naver-blog.png" style="max-height:460px" />

---

<img src="./modoo.png" style="max-height:460px" />

---

- 2015년 5월 RFC 7540 스펙 공표
...
- `javax.servlet.http.PushBuilder`
	- Servlet 4.0을 지원하는 Container에서만 사용가능
...
<img src="./diagram-legacy.png" style="max-height:350px" />
...
- Front Web server - Backend App 서버 간에는 h2c
	- 프로토콜 전환 비용 최소화

<img src="./diagram-ideal.png" style="max-height:280px" />
...
- Nginx-Tomcat 사이에는 HTTP 1.1 연결
	- HttpClient로 8080 포트에게 요청을 날리면 h2c 연결이 되는데도..
...
- 안정화 되더라도 Apache httpd가 h2에 어울릴지는 의문
	- 연결당 자원소모가 큰 구조.
	- Event MPM이 있지만 Nginx보다 성능에서 뒤지는 것으로 나옴. (Appendix 참조)

---

- Nginx : 1.9.5 이상에서 HTTP/2 지원
	- [1.9.9 미만에서는 upstream으로의 HTTP/1.1 연결에 버그가 있었음](https://trac.nginx.org/nginx/changeset/78b4e10b4367b31367aad3c83c9c3acdd42397c4/nginx/)
	- 서버 push 지원 안 함
	- Upstream (Backend server) 연결에 h2c를 지원 안 함
    	- 아직 구체적인 계획 없음
...
- Servlet 4.0의 Server push를 포기하면 익숙한 뒷단 구성을 살릴 수 있음.
	- 브라우저-(h2)-> Nginx -(http/1.1)-> Tomcat 구성
- nghttpx를 앞단에 두고 LINK 헤더를 이용한 push 자원 지정 방식도 가능

---

- 어차피 Server push 못 쓰니
	- h2를 위해 Tomcat 9, Spring 5 올릴 필요 없다.
- Java9 클라이언트
	- 인큐베이터이니 테스트 정도만?
- 내부망의 Server to Server 연결에서는 굳이 h2(with TLS)를 써야 하는지도 의문을 가져야 함.
...
<img src="./interstellar.jpg" style="max-height:420px" />
...
<img src="./server-push-result.png" style="max-height:420px" />
...
<img src="./diagram-h2-nginx.png" style="max-height:400px" />

---

뒷단 서버의 구성을 건드리지 않고 HA Proxy만 추가해서도 가능

<img src="./diagram-h2-on-legacy.png" style="max-height:400px" />
...
- nghttpx를 이용한 구성
	...
	- nghttpx와 Tomcat 중간에 Apache httpd 또는 Nginx를 넣을 수도 있음.

<img src="./diagram-h2-nghttpx.png" style="max-height:280px" />
...
<img src="./diagram-h2-demo2.png" style="max-height:300px" />
...
<img src="./diagram-h2-tomcat-native.png" style="max-height:300px" />

---

- 기존 서버 구성과 달라져서 고민해봐야 할 것
	- Health check 방식
	- 배포 시 서버를 제외하는 방식
...
- 내부망에서의 호출은 HTTP 1.1로 충분할 수 있음.
 	- OkHttp도 아직 h2c 미지원
- 외부 서버와 HTTPS + HTTP/2 호출을 해야 한다면
	- 현시점에서는 OkHttp 추천
...
- 더 효율적인 전송을 위해 제안된 스펙
	...
	- [An HTTP Status Code for Indicating Hints](https://tools.ietf.org/html/draft-ietf-httpbis-early-hints-05) : Client에게 힌트를 줄 때 쓸 수 있는 새로운 상태 코드

---

### Server to Server 연결에 h2c vs HTTP/1.1

- h2c의 multiplexing을 이용하면 Connection pool 구성이 어떻게 달라져야 할까?
...
- 클라이언트 : 더 다양한 프로그래밍 모델, 라이브러리가 난립하는 시대.
	- Spring 내에서도 RestTemplate, AsyncRestTemplate, WebClient
	- HTTP/2 지원 여부까지 포함하면 경우의 수가 더 많아짐.
	- 다양한 시도와 공유가 필요
...
- 바이너리 실행 코드(WebAssembly) + 바이너리 프로토콜(h2)
...
- 브라우저 위에서 바이너리 RPC 프로토콜을 실행할 수 있는 여지도 커짐
	...
	- 게임, 지도 등 고성능이 필요한 클라이언트 환경에서 고려해볼 만함
...
- worker MPM 을 바탕으로 하고 있기 때문에 context switching이 발생하는 구조
...
- h2c 연결에 쓰이는 [StreamProcessor](https://github.com/apache/tomcat/blob/trunk/java/org/apache/coyote/http2/StreamProcessor.java#L209) 에서는 doPush() 메서드가 구현되어 있음.
