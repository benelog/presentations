...
## 발표자료
- https://benelog.github.io/presentations/20171126-http2-java/
...
- RestTemplate 예제
	- [H2ClientTest.java](src/test/java/net/h2demo/H2ClientTest.java) : RestTemplate + (OkHttp, Netty, JDK HttpURLConnection) 
- JShell + Java9 HttpClient
	- [h2demo.jsh](h2demo.jsh) : 8080 포트로 h2c연결이 되는지 확인할수 있는 테스트 스크립트
	- [dev-h2demo.jsh](dev-h2demo.jsh) : 443 포트로 인증서를 검증하지 않고 연결하는 테스트 스크립트
-  [capture-8080.sh](capture-8080.sh) : tshark로 8080 포트 모니터링
)

JShell로 작성한 `dev-h2demo.jsh` 와 같은 파일들은 Linux 명령행에서 `./dev-h2demo.jsh`와 같이 바로 실행할 수 있다.

### Spring Boot 2.0.M6 + Tomcat 9, Server push
- commit `e7e62b2` : 버전을 올리고 server push를 추가하는 commit (아래 파일들에 반영된 상태)
- [pom.xml](pom.xml)
- [Http2DemoApplication.java](src/main/java/net/h2demo/Http2DemoApplication.java) : Spring Boot2에 맞도록 h2 프로토콜 업그레이드 선언
- [HomeController.java](src/main/java/net/h2demo/HomeController.java) : Server push 추가


이 디렉터리의 파일이 이 버전이므로, 그대로 Tomcat을 올리면 해당 버전의 예제가 실행된다.
```
./mvnw spring-boot:run
```
